...
def train_doc2vec_model(labeled_sentences):
    model = Doc2Vec(dm=0, dm_mean =1, size=200, window=10, min_count=5, workers=20, alpha=0.025, min_alpha=0.025)
...
def run(training_fn, label_fn, testing_fn, pred_fn):
    ...
    # title_vectorizer = TfidfVectorizer(lowercase=True,max_df=1.0, min_df=1,use_idf=True,ngram_range=(1,1))
    ...
    # print 'obatined vectors for individual modalities: '' title: {}, cat1: {}, cat2: {}, cat3: {}, desc:{}'.format(
    ...
    run_n_times_with_cv(train_full_vects, training_labels, Classifier, Params)
    # run_n_times_with_cv(train_title_vects, training_labels, Classifier, Params)

    best_model = get_best_model_from_trainingset(train_title_vects, training_labels,
                                                 Classifier, Params)
    # Preds = best_model.predict_proba(test_title_vects)
    Preds = best_model.predict_proba(test_full_vects)
    y_preds = Preds[:, 1]
