...
print (x_text[1:8],type(x_text),type(x_text[1]))
raw_input()

def mysentences (data):
    sentences = []
    for str in data:
...
# Build vocabulary
''''
sentences = mysentences(x_text)
# train word2vec on the sentences
model = Word2Vec(sentences, size=100, window=5, min_count=5, workers=10)
x = model.wv
'''
lable_sen = label_sentences(x_text)
train_model = train_doc2vec_model(lable_sen)
x = np.array([train_model.docvecs[i] for i in range(0, len(x_text))])
print (x[1:8],type(x),len(x[1]))
raw_input()
...
print("Vocabulary Size: {:d}".format(200))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        train_model.save(os.path.join(out_dir, "vocab"))
